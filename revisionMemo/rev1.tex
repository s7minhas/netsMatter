...
\begin{enumerate}
	...
	\begin{itemize}
	\item  \textcolor{blue}{ \emph{ What we think: True DGP: $Y = B0 + XB1 + a + b +$ 3rd order stuff + e
	\item What most do: $Y = \hat{B0} + X\hat{B1} + e$
	\item Something like if we have dependencies then $E(\hat{B1}) = E(B1) + cov(B1,$ other stuff)
	\item SM: needs to add more here to indicate how the existing simulation section is meant to deal with this issue ... onus is on SM to bring in the problems that we highlight in the simulation section to section 2, so readers are clearer on the problem that we are trying to address
	}}
	\end{itemize}
\end{itemize}
	...
	\begin{itemize}
		\item \textcolor{blue}{ \emph{
		We have edited P8 to read: Non-iid observations in relational data result from the fact that there is a complex structure underlying the dyadic events or processes we observe. We have also clarified the goals of the paper in the earlier sections.
		\item Here we have clarified that our study does not address causal identification but instead focuses on providing unbiased estimates of covariates, calibrated confidence intervals that reduce inferential error, and improved predictive performance.  Though we understand these goals are not always central to every political scientists aim, we do believe they relate closely to the field of IR (as demonstrated by the studies we replicate) and a growing interest in forecasting and machine learning in Political Science. (CITE?) [add reference to DGM JOP in which AME beat random forest]
		\item For part (a), we have clarified that our study does not address causal identification. We instead focuses on providing unbiased estimates of covariates, which improves inference for observational data.
		\item As for (b), the reviewer raises an important point. How well does AME work out when the DGP does not abide by parametric assumptions...relates to SM's to-do below.
		\item Finally, we understand these goals are not always central political science research, but we believe that prediction is both a valuable metric to evaluate model performance as well as a goal for scholars in and of itself, as in \citet{people}.
		}}
	\end{itemize}
