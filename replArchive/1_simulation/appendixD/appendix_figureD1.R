...
# viz
ggBiasPlot = function(varName, figLabel, h=4, w=8){
	if(varName!='all'){
		g=ggplot(
	...
	ggsave(g, height=4, width=8,
		file=paste0(graphicsPath, 'appendix_figureD1',figLabel,'.pdf'),
		device=cairo_pdf
		)
	return(g)
}
...
#
print(ggBiasPlot('all', figLabel=figNames[ii], h=12, w=8))
}
##############################
