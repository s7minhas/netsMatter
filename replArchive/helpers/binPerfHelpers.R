...
####################################################################
#
ggPerfCurves = function(predDfs, suffix, plotPath, cutBy=0.001){
  ...
  modLabs = unlist(lapply(predDfs, function(x){unique(x$model)}))
  rocData$model = factor(rocData$model, levels=modLabs)
  prData$model = factor(prData$model, levels=modLabs)
