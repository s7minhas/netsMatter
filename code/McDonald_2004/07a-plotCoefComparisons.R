...
## Load in the data
## This data does not have intercepts estimated
...
## commented out 7/31 becuase only want to report k=2 and k=3 in plots
## for paper
## AME K0
...
pDat$prezvar = factor(pDat$prezvar,levels =
    rev(levels(pDat$prezvar)))##this is needed b/c of the axis orientation


length(ivs)## 15

pDat$group[pDat$prezvar %in% c(ivs[1:5])] = 1
...
## Remove splies and intercepts:

pDatPrez <- pDat[!(pDat$prezvar=="Spline0"
                   |pDat$prezvar=="Spline1"
                   | pDat$prezvar=="Spline2"
                   |pDat$prezvar=="Spline3"
                 | pDat$var=="(Intercept)"
                 |pDat$var=="rho"),]
...
ggCoef(data = pDatPrez) ;
ggsave(filename = paste0(resultsPath, 'McDonald_coefs.pdf'), device = cairo_pdf, width=7, height=7)

## Just relatively small effects:

pDatSmall <- pDatPrez[(pDatPrez$mean <=5 & pDatPrez$mean >= -5),]

ggCoef(data = pDatSmall) ;
ggsave(filename = paste0(resultsPath, 'McDonald_SmallCoefs.pdf'),
