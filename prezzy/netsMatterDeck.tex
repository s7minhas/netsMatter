...
\title[AMEN]{Taking Dyads Seriously\thanks{Based upon joint works with Michael Ward (Duke University), Cassy L. Dorff (University of New Mexico), Max Gallop (University of Strathclyde), Juan Tellez (Duke University), Howard Liu (Duke University), \& Margaret Foster (Duke University). 
}}
...
\date{\today\\ 2018 Asian Political Methodology Meeting\\Asia Center\\Seoul National University\\Seoul, Korea}
...
\frametitle{Abstract}
\begin{itemize}
\item  많은 국제관계학 연구는 양자 (dyad) 관계를 다룬다. 그러나 대부분의 경우 양자관계의 관찰은 통계학에서 요구되는 조건부 독립성 기준을 만족시키지 못한다. 결과적으로, 그러한 연구들은 종종 편향된 계수 값과 잘못 계산된 표준 오차 값을 산출하게 된다. 이 편차들은 모수적 통계 모형을 이용하여 수치를 구함에 있어 중대한 결과의 차이를 가져온다. 
\item 본 연구는 회귀분석에 근거한 방법론적 대안을 제시하여 양자관계 분석을 까다롭게 하는 종속성을 처리하는 방법을 소개한다. 또한, 시뮬레이션을 통해 차용된 모형이 어떻게 종속성을 처리하는지를 보여준다. 본 연구는, 기존의 통상적 접근 방식과 제시된 새로운 방식을 비교하기 위하여 잘 알려진 다섯 연구의 기존 모형을 복제 및 재검토 (replicate) 하였다.
\item 통상적 방법은 주요 변수의 효과를 과장하고, 통계적 불확실성은 과소 추산함으로써, 연구자들로 하여금 해당 변수의 실질적 중요도에 대해 오판하게 한다.
\item 반면, 본 연구에서 차용된 방법은 표본 외 예측 (out-of sample) 교차 검증법에서 우세한 결과를 보였다. 따라서, 기존의 방법과 비교하였을 때 차용된 방법은 예측 모형으로의 활용도가 더 높고, 주요 결과 이외의 데이터 생성 과정을 모형화하는 데에 더욱 유용하다. 
...
Thank you!  Thanks especially to Jong Hee Park and the Organizing Committee.
